...
class QueueRepo(BaseRepo):
    ...
    async def get_by_chat_id(self, chat_id: int) -> model | None:
        ...
        Args:
            chat_id: The chat ID associated with the queue.

        Returns:
            The queue instance if found, otherwise None.
        """
    ...
    async def get_with_positions(self, id: UUID) -> model | None:
        ...
        Args:
            id: The UUID of the queue.

        Returns:
            The queue instance with positions if found, otherwise None.
        """
    ...
    async def get_position(
        ...
        Returns:
            The position instance if found, otherwise None.
        """
    ...
    async def add_position(self, queue: model, user_id: UUID) -> int:
        ...
        Returns:
            The position of the user in the queue, or -1 if they are
            already in it.
        """
    ...
    async def remove_position(
        ...
        Args:
            queue: The queue instance.
            user_id: The UUID of the user whose position should be removed.
        ...
        if not target:
            self.logger.info(f"User {user_id} not found in queue {queue.id}")
            return False, None

        next_user_id = (
            queue.positions[1].user.tgid
            if len(queue.positions) > 1 and target.position == 0
            else None
        )
