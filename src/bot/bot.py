...
It retrieves settings from the `getSettings()` function, creates a `Bot` instance
with the specified token and parse mode, and initializes a `Dispatcher` with
in-memory storage.
...
The resulting `bot` and `dp` objects are the main instances used for bot operation.
"""
